...
class StatusHandler(tornado.websocket.WebSocketHandler):
    ...
    def received_message(self, m):
        payload = m
        dat = msgpack.unpackb(payload,raw=False)
        ...
        for i in range(agent_count):
            img = Image.open(io.BytesIO(bytearray(dat['image'][i])))
            img = img.convert('L')
        ...
        if args.model == 'None':
            actions,actions_for_pass = self.agent.select_action(observation,end_episode)
            self.agent.learn()
            self.agent.store_experience(observation, actions, reward, end_episode)

        #print('action:',actions,'\n reward:', reward)
        #self.send_action(action_lst)
        #actions = [[1.5,1.5],[2.0,2.0],[1.7,0.3]]
        #print(actions_for_pass)
        print(reward)
        print(actions_for_pass)
        self.send_action(actions_for_pass)
