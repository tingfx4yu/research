...
class replay_memory():
    ...
    def sample(self,batch_size,priority = 0.5):
        ...
        rand_idx = np.random.choice(range(len(self.memory)-num_agents), batch_size, p = probs,replace = False)
...
class Agent():
    ...
    def step(self,state_cnn,action,reward,done,weight_importance = 0.5, priority = 0.5):
    ...
    def select_action(self,obs,done):
        self.step_counter += 1
        a = 0.9
        b = (5000 - max(0, self.step_counter - 100))
        threshold = 0.1 + max(0, a * b / 5000)
    ...
    def learn(self,exp_batch, weight_importance = 0.5):
        ...
        weights = (((1/len(self.buffer))*(1/probs))**weight_importance).cuda()
...
'''
    def store_experience(self,obs,action,reward,done,p):
        state_cnn = self.get_obs_cnn(obs)
        for i in range(num_agents):
            self.memory.add(state_cnn[i],action[i],reward[i],done)
            self.priority.add(p)
'''
